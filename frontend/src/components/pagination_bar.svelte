...
<div>
    <button on:click={() => current_page > 1 && (updateCurrentPage(current_page - 1))}>&lt; Prev</button>
    ...
    {#if current_page !== max_page}
        ...
        {#each calculatePageRange(current_page, current_page + 5) as n}
            <button class={n === current_page && 'current-page'} on:click={() => (updateCurrentPage(n))}
                >{n}</button
            >
        {/each}
        {#if max_page - current_page > 1}
    ...
    {#if 1 !== max_page}
        ...
        >
    {/if}
    <button on:click={() => current_page < max_page && (updateCurrentPage(current_page + 1))}>Next &gt;</button>
</div>
